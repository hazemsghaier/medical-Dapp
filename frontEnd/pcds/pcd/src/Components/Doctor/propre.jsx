...
import { getAppointementDocteur } from '../../api/rendez-vous';
import './propre.css';
...
const Propre = () => {
  ...
  connectedWallet();
    const fetchAppointments = async () => {
      try {
        const result = await getAppointementDocteur();
        const confirmed = result.filter(item => item.statue === true).map(item => ({
          ...
          name:  item.patientEmail, // Assuming you need to provide a name for the doctor.
          status: item.statue
        }));
